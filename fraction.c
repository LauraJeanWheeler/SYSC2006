# SYSC2006
Foundations of Imperative Programming in C
...
void make_fraction(int a, int b, fraction_t *new_fraction)
{
    ...
    if (new_fraction->den == 0){
            printf("You silly goose, don't divide by zero!");
        exit(1);
    }
    reduce(new_fraction);
...
{
int f_add_num = ((pf1->num*pf2->den)+(pf1->den*pf2->num));
int f_add_den = pf1->den*pf2->den;

make_fraction(f_add_num,f_add_den,sum);
...
void multiply_fractions(const fraction_t *pf1, const fraction_t *pf2, 
	                    fraction_t *product)
{
    int f_multiply_num = pf1->num*pf2->num;
    int f_multiply_den = pf1->den*pf2->den;

    make_fraction(f_multiply_num,f_multiply_den, product);
}
