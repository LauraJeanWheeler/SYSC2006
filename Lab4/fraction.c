...
{
    int f_add_num = ((f1.num*f2.den)+(f1.den*f2.num));
    int f_add_den = (f1.den*f2.den);

    return make_fraction(f_add_num,f_add_den);
...
{
    int f_mult_num = (f1.num*f2.num);
    int f_mult_den = (f1.den*f2.den);

	return make_fraction(f_mult_num, f_mult_den);
}
